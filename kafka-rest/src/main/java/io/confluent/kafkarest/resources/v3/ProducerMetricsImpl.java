...
public final class ProducerMetricsImpl implements ProducerMetrics {
  ...
  ProducerMetricsImpl(KafkaRestConfig config, Time time) {
    this(config, time, ImmutableMap.of());
  }

  ProducerMetricsImpl(KafkaRestConfig config, Time time, Map<String, String> metricsTags) {
  ...
  private Metrics createMetrics(RestMetricsContext metricsContext, Time time) {
