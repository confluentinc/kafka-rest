...
public class KafkaConsumerManagerTest {
  ...
  public void testBackoffMsControlsPollCalls() throws Exception {
      ...
      CopyOnWriteArrayList<Long> pollTimestampsNanos = new CopyOnWriteArrayList<>();
      ...
      assertTrue(
              String.format(
                      "Expected at most 12 poll calls, but got %d instead.", pollTimestampsNanos.size()),
              pollTimestampsNanos.size() <= 12);
      ...
      for (int i = 0; i < pollTimestampsNanos.size() - 1; i++) {
          ...
          assertTrue(
                  String.format(
                          ...
                          pollTimestampsNanos.get(i), lastTimestampNanos, smallWindowCount),
                  smallWindowCount <= 2);

          assertTrue(
                  String.format(
                          "Expected at least 1 poll call in window (%d, %d), but got none instead.",
                          pollTimestampsNanos.get(i), pollTimestampsNanos.get(i + 1)),
      ...
      long timeoutTimestampNanos = pollTimestampsNanos.get(0) + timeoutMillis;
      assertTrue(
              String.format(
                      "Expected at least 1 poll call in window (%d, %d], but got none instead.",
