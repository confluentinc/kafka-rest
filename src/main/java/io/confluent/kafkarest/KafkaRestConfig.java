...
public class KafkaRestConfig extends RestConfig {
  ...
  public static final String SIMPLE_CONSUMER_MAX_POLL_RECORDS_CONGIG = "simpleconsumer.max.poll.records";
  private static final String
    SIMPLE_CONSUMER_MAX_POLL_RECORDS_DOC =
    "Maximum number of records that can be fetched by a single consumer poll request. Since desired number "
     + "of fetched records can't be defined for every poll request the consumer can poll more records than"
     + "necessary (extra records are stored in cache.). The value of this property defines maximum number of excess records fetched by a single "
     + "user request. The grater value means grater network overhead. It may be reasonable if user retrieves "
     + "records from specified partition increasing offset sequentially. The smaller value suits better when "
     + "the user consumes records in a random fashion."
     + "Use 0 for no max value allowed by kafka consumer 2147483647";
  public static final String SIMPLE_CONSUMER_MAX_POLL_RECORDS_DEFAULT = "100";
