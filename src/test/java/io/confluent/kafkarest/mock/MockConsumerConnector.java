...
public class MockConsumerConnector implements ConsumerConnector {
  ...
  public Map<String, List<KafkaStream<byte[], byte[]>>> subscribedStreams =
   ...
   */
  public MockConsumerConnector(Time time, String clientId,
                               Map<String, List<Map<Integer, List<ConsumerRecord>>>> streamDataSchedules,
  ...
  public <K, V> Map<String, List<KafkaStream<K, V>>> createMessageStreams(
      Map<String, Integer> stringIntegerMap, Decoder<K> kDecoder, Decoder<V> vDecoder) {
  ...
  public Map<String, List<KafkaStream<byte[], byte[]>>> createMessageStreams(
    ...
    for (Map.Entry<String, Integer> topicEntry : topicCountMap.entrySet()) {
      ...
      for (int i = 0; i < topicEntry.getValue(); i++) {
        streams.add(new KafkaStream<byte[], byte[]>(
