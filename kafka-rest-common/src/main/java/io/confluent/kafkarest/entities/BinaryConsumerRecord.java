...
public class BinaryConsumerRecord extends ConsumerRecord<byte[], byte[]> {
  ...
  public BinaryConsumerRecord(@JsonProperty("topic") String topic,
    ...
    try {
      ...
      throw new ConstraintViolationException("Record value contains invalid base64 encoding",
  ...
  private static byte[] decodeBinary(String binary, String field) {
    try {
      ...
      throw new ConstraintViolationException("Record " + field + " contains "
              + "invalid base64 encoding", Collections.emptySet());
    }
  }
}
